...
class SimulationController extends BaseController {
	...
	public function chartResults(){
		return View::make('simulation.charts');
	}

	public function server1(){
		$events     = Evento::all();
		$response    = array(array('0',0));
	
		foreach ($events as $event) {
			$response[]    = array($event['event_time'],$event['util_s1_flag']);
		}
	...
	public function server2(){
		...
		foreach ($events as $event) {
			$response[]    = array($event['event_time'],$event['util_s2_flag']);
		}

		return Response::json($response);
	}

	public function req_col_1(){
		$events     = Evento::all();
		$response    = array(array('0',0));
	
		foreach ($events as $event) {
			$response[]    = array($event['event_time'],$event['req_cola1']);
		}

		return Response::json($response);
	}

	public function req_col_2(){
		$events     = Evento::all();
		$response    = array(array('0',0));
	
		foreach ($events as $event) {
			$response[]    = array($event['event_time'],$event['req_cola2']);
		}
	...
	public function system(){
		...
		foreach ($events as $event) {
			$response[]    = array($event['event_time'],$event['req_sistema']);
		}
